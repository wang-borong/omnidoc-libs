...
local renderDir = "figures"
local contdir = "diagascode"
...
function renderImg(block)
  ...
  local sha1ofcont = pandoc.sha1(block.text) .. "_" .. engine
	local sha1file = renderDir .. "/" .. sha1ofcont
  local contfile = contdir .. "/" .. sha1ofcont
	local fname = sha1file .. "." .. filetype
	local cname = contfile .. ".txt"

  if block.identifier then
    local id = block.identifier
    local id_str = id:gsub('fig:', '', 1)
    -- logging.output("id", id_str)
    fname = renderDir .. "/" .. id_str .. "." .. filetype
    cname = contdir .. "/" .. id_str .. ".txt"
  end
...
function CodeBlock(block)
	if block.attributes["subfig"] == nil then
		...
		if img.t == "Image" then
      -- logging.temp('img1', pandoc.Para{ img })
			return pandoc.Para{ img }
		else
      -- logging.temp('img2', img)
			return img
		end
	end
end
